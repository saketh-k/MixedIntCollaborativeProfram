...
class TerrainPlanner:
    ...
    def optimize(self, switch_cost):
        ...
        t1 = [lp.LpVariable(f"t1_{i}",cat="Integer") for i in range(self.Ns)]
        t2 = [lp.LpVariable(f"t2_{i}", cat = "Integer") for i in range(self.Ns)]
        ...
        for i in range(self.Ns):
            ...
            prob += t2[i] == self.robot1_eff*self.ds* (self.robot2_mass * z1[i] + (self.robot1_mass + self.robot2_mass) * z3[i])
            prob += z1[i] + z2[i] + z3[i] == 1

        prob += lp.lpSum(t1) + lp.lpSum(t2) + switch_cost * lp.lpSum(switches)
        prob.solve()
